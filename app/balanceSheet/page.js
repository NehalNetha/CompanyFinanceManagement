...
const getTotalExpenses = async () => {
  try{
    const res = await fetch("http://localhost:3000/api/expenses/totalExpenses", {
      cache: "no-store"
    })
...
const getTotalOrders = async () => {
  try {
    const res = await fetch("http://localhost:3000/api/createInvoice/totalOrders", {
      cache: "no-store"
    })
    if (!res.ok) {
...
const getTotalPurchases = async () => {
  try {
    const res = await fetch("http://localhost:3000/api/createPO/totalPurchases", {
      cache: "no-store"
    })
    if (!res.ok) {
...
const getTotalInventory = async () => {
  try {
    const res = await fetch("http://localhost:3000/api/inventory/totalInventory", {
      cache: "no-store"
    })
    if (!res.ok) {
...
const getTotalSalary = async () => {
  try {
    const res = await fetch("http://localhost:3000/api/payEmployee/totalSalary", {
      cache: "no-store"
    })
    if (!res.ok) {
  ...
  const [cash, setCash] = useState(200000)
    ...
    useEffect(() => {
      ...
      const fetchPurchases = async () =>{
        ...
        setCash((prevCash) => prevCash - data.totalPurchases)
      }
      ...
      const fetchSalary = async () =>{
        ...
        setTotalSalary(data.totalDisbursment)
        ...
        <div className='flex flex-row justify-evenly border-2 border-black w-[70rem] m-8 p-8'>
            <div className="flex flex-col gap-11 justify-center ml-11 mt-11">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {renderRow("Inventory", totalInventory)}
                    <hr  />

                    <div className='font-semibold'>
                      {renderRow("Total Current Assets", cash+totalOrders+totalInventory)}
                      <hr />
                    </div>
    ...
    function renderRow(label, price) {
      ...
      return (
        <div className="flex flex-row gap-9">
          ...
          <p className='ml-11'>{formattedPrice}</p>
        </div>
      );
}
